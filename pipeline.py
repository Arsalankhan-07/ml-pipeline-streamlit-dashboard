...
def inject_css(dark: bool):
    if dark:
        surface="#161B22"; card="#21262D"; border="#30363D"
        text="#E6EDF3"; muted="#8B949E"; accent="#58A6FF"
        accent2="#3FB950"; accent3="#FF7B72"; accent4="#D2A8FF"
        grad1="#1F6FEB"; grad2="#238636"
    else:
        surface="#FFFFFF"; card="#F0F2F5"; border="#D0D7DE"
        text="#1F2328"; muted="#656D76"; accent="#0969DA"
        accent2="#1A7F37"; accent3="#CF222E"; accent4="#8250DF"
        grad1="#0969DA"; grad2="#1A7F37"

    st.markdown(f"""
<style>
@import url('https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;600&family=Sora:wght@400;600;700;800&family=Inter:wght@300;400;500;600&display=swap');
:root{{
...
def get_download_link(obj, filename, label):
    ...
    return (
        f'<a href="data:file/pkl;base64,{b64}" download="{filename}">'
        ...
        f'font-weight:600;cursor:pointer;font-size:0.84rem;">⬇ {label}</button></a>'
    )
...
<div style="text-align:center;padding:0.5rem 0 1rem;">
  ...
  <div style="font-size:0.7rem;color:#8B949E;margin-top:0.2rem;">
    CS-303B · CA-2 Project
  </div>
</div>""", unsafe_allow_html=True)
        st.markdown("---")

        # dark mode toggle — unique key
        dark = st.toggle("🌙 Dark Mode", value=st.session_state.dark_mode, key="sb_dark_toggle")
        ...
        prob = st.selectbox(
            "prob_type_select", ["Classification", "Regression"],
        ...
        st.markdown(
            f'<div style="background:{col}22;border:1px solid {col}55;border-radius:8px;'
            f'padding:0.45rem 0.75rem;font-size:0.78rem;color:{col};font-weight:600;">'
            f'{"🔵 Classification" if prob=="Classification" else "📈 Regression"}</div>',
            unsafe_allow_html=True,
        )
        ...
        step_names = [
            "Data Input","EDA","Preprocessing","Outlier Detection",
            "Feature Selection","Data Split","Model Selection",
            "Training & CV","Metrics","HP Tuning",
        ]
        active = st.session_state.active_step
        for i, sname in enumerate(step_names, 1):
            icon  = "✅" if i < active else ("▶" if i == active else "○")
            color = "#3FB950" if i < active else ("#58A6FF" if i == active else "#8B949E")
            w     = "700" if i == active else "400"
            st.markdown(
                f'<div style="display:flex;align-items:center;gap:0.45rem;padding:0.22rem 0;'
                f'font-size:0.79rem;color:{color};font-weight:{w};">'
                f'<span>{icon}</span><span>{i}. {sname}</span></div>',
                unsafe_allow_html=True,
            )
        ...
        st.markdown(
            ...
            'Streamlit · sklearn · plotly · pandas</div>',
            unsafe_allow_html=True,
        )
...
<div class="hero-banner">
  ...
  <div class="hero-sub">
    AutoML · EDA · Preprocessing · Outlier Detection · Feature Engineering ·
    Model Training · Hyperparameter Tuning &nbsp;|&nbsp; Mode: <strong>{pt}</strong>
  </div>
</div>""", unsafe_allow_html=True)
...
def render_stepper():
    ...
    active = st.session_state.active_step
    pills  = "".join(
        f'<div class="step-pill {"done" if i<active else ("active" if i==active else "")}">'
        f'<span class="step-num">{"✓" if i<active else str(i)}</span>{name}</div>'
...
def step_data_input():
    ...
    uploaded = st.file_uploader(
        "Upload your CSV dataset", type=["csv"], key="s1_uploader"
    )
    ...
    if uploaded is None:
        ...
        with st.expander("💡 No dataset? Load a sample"):
            c1, c2 = st.columns(2)
            with c1:
                if st.button("🌸 Iris  (Classification)", key="s1_btn_iris"):
                    from sklearn.datasets import load_iris
                    st.session_state.df_raw    = load_iris(as_frame=True).frame
                    st.session_state.target_col = "target"
                    st.session_state.active_step = 2
                    st.rerun()
            with c2:
                if st.button("🏠 California Housing  (Regression)", key="s1_btn_cal"):
                    from sklearn.datasets import fetch_california_housing
                    st.session_state.df_raw    = fetch_california_housing(as_frame=True).frame
    ...
    with t1:
        n = st.slider("Rows to display", 5, min(100, len(df)), 10, key="s1_rows_slider")
    ...
    with t3:
        ...
        if num_df.shape[1] < 2:
            ...
            pca_cols = st.multiselect(
                "Features for PCA", num_df.columns.tolist(),
                default=num_df.columns.tolist()[:min(10, len(num_df.columns))],
                key="s1_pca_cols",
            )
            pca_dim = st.radio("Dimensions", [2, 3], horizontal=True, key="s1_pca_dim")
            if pca_cols and len(pca_cols) >= pca_dim:
                ...
                color_col  = st.selectbox(
                    "Colour by", ["None"] + df.columns.tolist(), key="s1_pca_color"
                )
                color_vals = df[color_col].astype(str) if color_col != "None" else None
                if pca_dim == 2:
                    fig = px.scatter(
                        x=C[:, 0], y=C[:, 1], color=color_vals,
                        labels={"x": f"PC1 ({ev[0]*100:.1f}%)", "y": f"PC2 ({ev[1]*100:.1f}%)"},
                        title="PCA 2D", template=tmpl(), color_discrete_sequence=PALETTE,
                    )
                else:
                    fig = px.scatter_3d(
                        x=C[:, 0], y=C[:, 1], z=C[:, 2], color=color_vals,
                        labels={"x": f"PC1 ({ev[0]*100:.1f}%)",
                                "y": f"PC2 ({ev[1]*100:.1f}%)",
                                "z": f"PC3 ({ev[2]*100:.1f}%)"},
                        title="PCA 3D", template=tmpl(), color_discrete_sequence=PALETTE,
                    )
                ...
                st.plotly_chart(
                    px.bar(ev_df, x="Component", y="Explained Variance %",
    ...
    with ca:
        target = st.selectbox(
            "target_col_select", df.columns.tolist(),
            index=len(df.columns) - 1,
            label_visibility="collapsed",
            key="s1_target_select",
        )
    with cb:
        if st.button("✅ Confirm & Continue →", key="s1_confirm_btn"):
            st.session_state.target_col  = target
            st.session_state.active_step = 2
...
def step_eda():
    ...
    t1, t2, t3, t4 = st.tabs(
        ["📉 Missing Values", "📊 Distributions", "🔥 Correlation", "🎯 Target"]
    )
    ...
    with t1:
        ...
        if miss.empty:
            ...
            st.plotly_chart(
                px.bar(miss, x="Column", y="Pct", text="Missing",
    ...
    with t2:
        ...
        if num_cols:
            ...
            for col in sel:
                ...
                fig.update_layout(template=tmpl(), showlegend=False, height=300,
        ...
        if cat_cols:
            ...
            for col in sel_c:
                ...
                st.plotly_chart(
                    px.bar(vc, x=col, y="Count", template=tmpl(),
    ...
    with t3:
        ...
        if num_df.shape[1] < 2:
            ...
            st.plotly_chart(fig, width="stretch")
            if st.checkbox("Show scatter matrix (top 5 cols)", key="s2_scatter_cb"):
                st.plotly_chart(
                    px.scatter_matrix(df[num_df.columns[:5].tolist()],
    ...
    with t4:
        if target and target in df.columns:
            if df[target].dtype == "object" or df[target].nunique() <= 20:
                ...
                with ca:
                    st.plotly_chart(
                        px.bar(vc, x=target, y="Count", template=tmpl(),
                               color_discrete_sequence=[PALETTE[0]],
                ...
                with cb:
                    st.plotly_chart(
                        px.pie(vc, names=target, values="Count", template=tmpl(),
                               title="Target Share", color_discrete_sequence=PALETTE),
                        width="stretch",
                    )
            else:
                st.plotly_chart(
                    px.histogram(df, x=target, template=tmpl(), marginal="violin",
                                 color_discrete_sequence=[PALETTE[0]],
                                 title="Target Distribution"),
                    width="stretch",
                )
        else:
            info("Select a target column in Step 1 first.")

    st.markdown("---")
    if st.button("✅ Continue to Preprocessing →", key="s2_continue_btn"):
...
def step_preprocessing():
    ...
    with cb:
        ...
        if mc.empty:
            ...
            st.dataframe(mc.rename("Missing"), width="stretch")

    if st.button("🚀 Apply Preprocessing", key="s3_apply_btn"):
        with st.spinner("Processing..."):
            ...
            le_dict = {}
            if cat_cols:
                X[cat_cols] = SimpleImputer(
                    strategy=cat_strat, fill_value="missing"
...
def step_outlier_detection():
    ...
    if df is None:
        warn("Complete preprocessing first."); return
    ...
    if method == "IQR":
        mult = st.slider("IQR multiplier", 1.0, 3.0, 1.5, 0.1, key="s4_iqr_mult")
        for c in feat_cols:
        ...
        cont = st.slider("Contamination", 0.01, 0.3, 0.05, 0.01, key="s4_cont")
        pred = IsolationForest(contamination=cont, random_state=42).fit_predict(
            df[feat_cols].fillna(0)
        )
        mask = pd.Series(pred == -1, index=df.index)

    elif method == "DBSCAN":
        eps   = st.slider("eps", 0.1, 5.0, 0.5, 0.1, key="s4_eps")
        min_s = st.slider("min_samples", 2, 20, 5, key="s4_min_s")
    ...
    if len(feat_cols) >= 2:
        fig = px.scatter(
            df, x=feat_cols[0], y=feat_cols[1],
    ...
    if n_out > 0:
        if st.checkbox("👁 Preview outlier rows", key="s4_preview_cb"):
            st.dataframe(df[mask].head(20), width="stretch")
        if st.button("🗑️ Remove Outliers from Dataset", key="s4_remove_btn"):
        ...
        success("No outliers found with current settings.")

    st.markdown("---")
    if st.button("✅ Continue to Feature Selection →", key="s4_continue_btn"):
...
def step_feature_selection():
    ...
    if target not in df.columns:
        warn(f"Target '{target}' not found in the cleaned dataframe."); return
    ...
    if method == "Variance Threshold":
        thresh = st.slider("Variance threshold", 0.0, 1.0, 0.01, 0.01, key="s5_var_thresh")
        sel    = VarianceThreshold(threshold=thresh)
        sel.fit(num_X.fillna(0))
        kept     = num_X.columns[sel.get_support()].tolist()
        selected = kept
        var_df   = pd.DataFrame({
            "Feature":  num_X.columns,
            "Variance": num_X.var().values,
            "Selected": num_X.columns.isin(kept),
        }).sort_values("Variance", ascending=False)
        st.plotly_chart(
            px.bar(var_df, x="Feature", y="Variance", color="Selected",
                   color_discrete_map={True: "#3FB950", False: "#FF7B72"},
                   title="Feature Variances", template=tmpl()),
            width="stretch",
        )

    elif method == "Correlation":
        ct    = st.slider("Min |correlation| with target", 0.0, 1.0, 0.05, 0.01, key="s5_corr_thresh")
        ...
        st.plotly_chart(
            px.bar(cdf, x="Feature", y="Correlation", color="Selected",
                   color_discrete_map={True: "#3FB950", False: "#FF7B72"},
        ...
        k     = st.slider("Top-K features", 1, max(1, len(num_X.columns)),
                          min(10, len(num_X.columns)), key="s5_ig_k")
        ...
        st.plotly_chart(
            px.bar(igdf, x="Feature", y="IG", color="Selected",
                   color_discrete_map={True: "#3FB950", False: "#FF7B72"},
    ...
    valid_defaults = [f for f in selected if f in X.columns]
    selected = st.multiselect(
        "Final features to use",
        options=X.columns.tolist(),
        default=valid_defaults,
        key="s5_feat_multiselect",
    )
    ...
    if st.button("✅ Confirm Features & Continue →", key="s5_confirm_btn"):
        if not selected:
            warn("Select at least one feature."); return
...
def step_data_split():
    ...
    if df is None or not feats:
        warn("Complete feature selection first."); return
    ...
    ca, cb = st.columns(2)
    with ca:
        test_size = st.slider("Test set size", 0.1, 0.5, 0.2, 0.05, key="s6_test_size")
    with cb:
        seed = st.number_input("Random seed", 0, 9999, 42, key="s6_seed")
    ...
    fig = go.Figure(go.Pie(
        labels=["Train", "Test"], values=[len(X_train), len(X_test)],
    ...
    if st.button("✅ Confirm Split & Continue →", key="s6_confirm_btn"):
...
def step_model_selection():
    ...
    opts = (
        ["Logistic Regression", "SVM", "Random Forest", "KMeans (Clustering)"]
        if pt == "Classification"
        else ["Linear Regression", "SVM", "Random Forest"]
    )
    name   = st.selectbox("Model", opts, key="s7_model_select")
    params = {}

    if name == "SVM":
        kernel = st.selectbox("Kernel", ["rbf", "linear", "poly", "sigmoid"], key="s7_svm_kernel")
        C      = st.number_input("C (regularization)", 0.01, 100.0, 1.0, 0.1, key="s7_svm_c")
        params = {"kernel": kernel, "C": C}

    elif name == "Random Forest":
        n_est = st.slider("Number of trees", 10, 500, 100, 10, key="s7_rf_trees")
        max_d = st.selectbox("Max depth", [None, 5, 10, 20, 50], key="s7_rf_depth")
        params = {"n_estimators": n_est, "max_depth": max_d}

    elif name == "KMeans (Clustering)":
        k      = st.slider("Clusters (K)", 2, 15, 3, key="s7_km_k")
        params = {"n_clusters": k}

    elif name == "Logistic Regression":
        C      = st.number_input("C (inverse regularization)", 0.01, 100.0, 1.0, 0.1, key="s7_lr_c")
        params = {"C": C}

    info(f"Selected: **{name}**")

    if st.button("✅ Confirm Model & Continue →", key="s7_confirm_btn"):
...
def step_training():
    ...
    if X_train is None:
        warn("Complete data split first."); return

    name   = st.session_state.model_name
    params = st.session_state.model_params or {}
    pt     = st.session_state.problem_type
    if not name:
        warn("Select a model first."); return

    st.markdown("### 🏋️ Step 8 · Training & K-Fold Cross Validation")
    k = st.slider("K (number of folds)", 2, 15, 5, key="s8_kfold_k")

    if st.button("🚀 Train Model", key="s8_train_btn"):
        model = build_model(name, params, pt)
        if model is None:
            warn("Could not build model."); return
        ...
        c1, c2, c3 = st.columns(3)
        c1.metric(f"Mean CV {scoring.upper()}", f"{scores.mean():.4f}")
        ...
        fig = px.bar(
            fold_df, x="Fold", y="Score",
            title=f"K-Fold CV Scores ({scoring})", template=tmpl(),
        ...
        success(f"Training complete! Mean {scoring}: **{scores.mean():.4f}**")
        st.session_state.active_step = 9
...
def step_metrics():
    ...
    if model is None:
        warn("Train a model first."); return
    ...
    if name == "KMeans (Clustering)":
        ...
        feats  = st.session_state.selected_features or []
        if len(feats) >= 2:
            fig = px.scatter(
            ...
            st.plotly_chart(fig, width="stretch")
        info("Unsupervised model — cluster assignments shown above.")
        return
    ...
    if pt == "Classification":
        ...
        gap = acc_tr - acc
        if gap > 0.15:
            warn(f"Possible **overfitting** — Train: {acc_tr:.4f} | Test: {acc:.4f} | Gap: {gap:.4f}")
        elif acc < 0.6:
            warn(f"Possible **underfitting** — Test accuracy: {acc:.4f}")
        else:
            success(f"Good generalisation — Train: {acc_tr:.4f} | Test: {acc:.4f}")
        ...
        gauge.update_layout(template=tmpl(), height=300,
        ...
        st.plotly_chart(
            px.imshow(cm, text_auto=True, template=tmpl(),
        ...
        fig.update_layout(barmode="group", template=tmpl(), height=300,
    ...
    # ── Regression ──
        ...
        gap = r2_tr - r2
        if gap > 0.2:
            warn(f"Possible **overfitting** — Train R²: {r2_tr:.4f} | Test R²: {r2:.4f}")
        elif r2 < 0.4:
            warn(f"Possible **underfitting** — R²: {r2:.4f}")
        else:
            success(f"Good fit — Train R²: {r2_tr:.4f} | Test R²: {r2:.4f}")

        mn  = float(min(float(np.min(y_test)), float(np.min(y_pred))))
        mx  = float(max(float(np.max(y_test)), float(np.max(y_pred))))
        fig = px.scatter(
            x=y_test, y=y_pred, template=tmpl(),
        ...
        st.plotly_chart(
            px.histogram(residuals, template=tmpl(), nbins=40,
    ...
    st.markdown(
        get_download_link(model, f"model_{name.replace(' ','_')}.pkl", "Download Model (.pkl)"),
        unsafe_allow_html=True,
    )
    st.markdown("---")
    if st.button("✅ Continue to Hyperparameter Tuning →", key="s9_continue_btn"):
        st.session_state.active_step = 10
...
def step_hp_tuning():
    ...
    if X_train is None or not name:
        warn("Complete training first."); return
    if name == "KMeans (Clustering)":
        info("Hyperparameter tuning is not applicable for KMeans."); return

    st.markdown("### ⚡ Step 10 · Hyperparameter Tuning")
    tune = st.radio(
        "Search strategy", ["GridSearchCV", "RandomizedSearchCV"],
        horizontal=True, key="s10_tune_radio",
    )
    ...
    grid = grids.get(name, {})
    if not grid:
        info(f"No hyperparameters to tune for {name}."); return

    st.markdown(f"**Search space for `{name}`:**")
    for k, v in grid.items():
        st.markdown(f"- `{k}`: {v}")

    cv_k   = st.slider("CV folds", 2, 10, 3, key="s10_cv_k")
    metric = "accuracy" if pt == "Classification" else "r2"

    if st.button("🔬 Run Hyperparameter Search", key="s10_run_btn"):
        base = build_model(name, {}, pt)
        if base is None:
            warn("Cannot build model."); return

        with st.spinner(f"Running {tune}..."):
            if tune == "GridSearchCV":
                searcher = GridSearchCV(base, grid, cv=cv_k, scoring=metric, n_jobs=-1)
            else:
                searcher = RandomizedSearchCV(
                    base, grid, n_iter=10, cv=cv_k,
        ...
        if pt == "Classification":
            ...
            fig = go.Figure()
            fig.add_trace(go.Bar(name="Before Tuning", x=["Accuracy"],
                                  y=[before_score], marker_color="#FF7B72"))
            fig.add_trace(go.Bar(name="After Tuning",  x=["Accuracy"],
                                  y=[after_score],  marker_color="#3FB950"))
            fig.update_layout(barmode="group", template=tmpl(), height=350,
                               title="Performance: Before vs After Tuning")
            st.plotly_chart(fig, width="stretch")
            delta = after_score - before_score
            (success if delta >= 0 else warn)(
                f"Accuracy {'improved' if delta >= 0 else 'changed'} by "
                f"**{delta*100:.2f}%** → {after_score:.4f}"
            )
        else:
            ...
            fig = go.Figure()
            fig.add_trace(go.Bar(name="Before", x=["R²", "MAE"],
                                  y=[br2, bmae], marker_color="#FF7B72"))
            fig.add_trace(go.Bar(name="After",  x=["R²", "MAE"],
                                  y=[after_r2, after_mae], marker_color="#3FB950"))
            fig.update_layout(barmode="group", template=tmpl(), height=350,
                               title="Performance: Before vs After Tuning")
            st.plotly_chart(fig, width="stretch")
...
def section_predict_new():
    ...
    up = st.file_uploader(
        "Upload new CSV for prediction", type=["csv"], key="s_pred_uploader"
    )
    if up:
        df_new  = pd.read_csv(up)
        missing = [c for c in feats if c not in df_new.columns]
        if missing:
            warn(f"Missing columns in new data: {missing}"); return
        X_new = df_new[feats].copy()
        sc    = st.session_state.scaler
        if sc:
            X_new = pd.DataFrame(sc.transform(X_new), columns=feats)
...
def main():
    ...
    tabs = st.tabs([
        "📂 Data Input", "🔍 EDA", "⚙️ Preprocessing", "🔎 Outliers",
        "🧬 Features",   "✂️ Split", "🤖 Model", "🏋️ Training",
        "📊 Metrics",    "⚡ Tuning",
    ])
